...
export const createObject = async (objectName, pkg, transport, sourceCode, callMcpTool, setStatus) => {
    try {
        ...
        // 3. Verify Creation & Get Object URL
        ...
        if (objSearchRes && objSearchRes.results) {
            const objMatch = objSearchRes.results.find(res =>
                res["adtcore:name"] === objectName.toUpperCase() &&
                (res["adtcore:type"] === 'CLAS/OC' || res["adtcore:type"] === 'CLAS')
            );
            if (objMatch) {
                objectUrl = objMatch["adtcore:uri"];
            }
        }

        if (!objectUrl) {
            throw new Error("Object verification failed: Could not find created object in system.");
        }
